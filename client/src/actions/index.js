    ...
    //   }
      ...
      export function getDogs() { //!! trae todos los dogs desde mi Api
        return async function (dispatch) {
            try {
                var json = await axios.get("/dogs/dogs");
...
export function getTemperaments() { //!!trae los temps de mi Api
	return async function (dispatch) {
		try {
			var json = await axios.get(`/dogs/temperament/`);
			return dispatch({
...
export function searchByName(name) {
	return async function (dispatch) {
		try {
			const json = await axios.get('/dogs/dogs?name=' + name);
...
export function postDog(payload) { // el payload me llega del form, es el obj a crear en la tabla
	...
	return async function () {
        ...
        var json = await axios.post(`/dogs/dogs`, payload); //le paso x BODY el obj creado en el form
...
export function getDogDetail(id) {
	return async function (dispatch) {
		try {
			var json = await axios.get(`/dogs/dogs/` + id);
